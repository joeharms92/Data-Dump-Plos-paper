...
void            TossDataAllHists(TString, TRandom3*, TH1D*);
...
void TossData(TH1D*, TRandom3*, TH1D*, Double_t, Double_t);
...
int main(int argc, char * argv[]) 
{
  ...
  TString outputFileName = "~/G4/ANSIsimulations/CNR_Zeff/ReconImages/imgs_lenrRawDataToss"+TString::Itoa(demultiplier,10)+".root";
    ...
    TossDataAllHists(histOutFilePath, rando, efficencyHist);
...
void TossDataAllHists(TString path, TRandom3* rng, TH1D* effHist)
{
  for (Int_t sliceIter = 0; sliceIter < numSlices; sliceIter++){
    cout << sliceIter << endl;
    TFile * outputHistFile = new TFile(path + "Slice" + TString::Itoa(sliceIter, 10) + "_toss" + TString::Itoa(demultiplier, 10) + ".root", "RECREATE");
    ...
    for (Int_t detIter = 0; detIter < numDets; detIter++){
      ...
      TossData(hist_run0,rng, effHist, low_lower, high_upper);
      TH1D * hist_run1 = (TH1D*)histFile_run1->Get("Det" + TString::Itoa(detIter, 10));
      TossData(hist_run1,rng, effHist, low_lower, high_upper);
...
TH2D* Transmission(TString whichEnergy, TString path)
{
  ...
  for (Int_t i = 0; i < 14; i++){
    TFile * ffFile = new TFile(path + "Slice" + TString::Itoa(ffSliceNumbers[i], 10) + "_toss" + TString::Itoa(demultiplier, 10) + ".root");
    for (Int_t detIter = 0; detIter < 2*numDets; detIter++){
      TH1D* ff = (TH1D*)ffFile->Get("Det"+TString::Itoa(detIter, 10) );
  ...
  for (Int_t sliceIter = 6; sliceIter < numSlices - 7; sliceIter++){
    TFile * histFile = new TFile(path + "Slice"+TString::Itoa(sliceIter,10) + "_toss" + TString::Itoa(demultiplier, 10) + ".root");
    for (Int_t detIter = 0; detIter < 2*numDets; detIter++){
...
void Plot(TH2D* tTotal, TH2D* tLow, TH2D* tHigh,  TH2D* ratio, TString path)
{
  ...
  c1->SaveAs(path+ "lenrTTotalRawDataToss"+TString::Itoa(demultiplier,10)+".pdf");
  c2->SaveAs(path+ "lenrTLowRawDataToss"+TString::Itoa(demultiplier,10)+".pdf");
  c3->SaveAs(path+ "lenrTHighRawDataToss"+TString::Itoa(demultiplier,10)+".pdf");
  c4->SaveAs(path+ "lenrRatioRawDataToss"+TString::Itoa(demultiplier,10)+".pdf");
}


void TossData(TH1D* hist, TRandom3* rng, TH1D* effHist, Double_t lowerBound, Double_t upperBound)
{
  Int_t nBins = hist->GetNbinsX();
  Double_t reduction = 1./(((Double_t)demultiplier)*36.4496626);
