...
int main(int argc, char** argv)
{
  ...
  for ( G4int i=1; i<argc; i=i+2 ) {
    ...
    else if ( G4String(argv[i]) == "-steelZ" ) steelThickness = G4UIcommand::ConvertToDouble(argv[i+1]);
...
G4cout << beam << endl;
  ...
  = new beamDoseDetectorConstruction(steelThickness);
  ...
  beamDoseActionInitialization* actionInitialization
    = new beamDoseActionInitialization(detConstruction, beam, steelThickness, runID);
...
void MoveToLower(G4String* aString)
{
  for (string::iterator i = aString->begin(); i < aString->end(); i++){
    *i = tolower(*i);
  }
}