...
int main()
{
	TString basepath = "~/G4/beamDose/MIRDPhantomDose/";
	...
	for (Int_t i = 0; i < 6; i++){
		TFile * file1 = new TFile(histPath + "BodyDose" + sources[i] +"_run1.root");
		TH1D * baseHist = (TH1D*)file1->Get("BodyDose");
		baseHist->SetName(sources[i] + "BodyDose");
		for (Int_t runNo = 2; runNo <= 10; runNo ++){
			TFile * file2 = new TFile(histPath + "BodyDose" + sources[i] +"_run" + TString::Itoa(runNo, 10) + ".root");
